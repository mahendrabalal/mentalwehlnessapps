-- Migration: Assessment Reminder Preferences
-- Description: User preferences for retake reminders with notification scheduling
-- Created: 2025-01-18
-- Best Practice: GDPR/HIPAA compliant notification system with granular opt-in/opt-out
...
CREATE TABLE IF NOT EXISTS assessment_reminder_preferences (
  ...
  -- Scheduling
  preferred_time_of_day TIME DEFAULT '09:00:00', -- User's preferred notification time
  ...
  total_reminders_acted_on INTEGER DEFAULT 0, -- User completed assessment after reminder
  ...
  auto_adjust_frequency BOOLEAN DEFAULT TRUE, -- Adjust based on user behavior
  quiet_hours_start TIME DEFAULT '22:00:00',
  quiet_hours_end TIME DEFAULT '08:00:00',
  pause_until TIMESTAMPTZ, -- Temporarily pause reminders

  -- Metadata
  created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),

  -- Ensure one preference per user per assessment type
...
CREATE TABLE IF NOT EXISTS assessment_reminder_queue (
  ...
  -- A/B testing support
  variant_id UUID, -- Link to ab_test_experiments if A/B testing reminders
...
-- Only system can insert/update queue (via service role)
...
RETURNS TRIGGER AS $$
BEGIN
  -- Only create if this is the first (baseline) assessment
...
BEGIN
  -- Find all preferences that need reminders scheduled
  FOR pref_record IN
    SELECT *
  ...
  LOOP
    ...
    -- Update preference with new next_reminder_date
...
BEGIN
  ...
  -- Disable reminders based on token scope
