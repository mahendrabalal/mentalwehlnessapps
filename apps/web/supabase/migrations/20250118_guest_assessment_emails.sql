-- Migration: Guest Assessment Email Captures
...
-- Policy: Only authenticated users can view their own submissions if converted
...
CREATE POLICY "Admins can view all guest emails"
  ON guest_assessment_emails
  FOR SELECT
  TO authenticated
  USING (
...
CREATE POLICY "Admins can update guest emails"
  ON guest_assessment_emails
  FOR UPDATE
  TO authenticated
  USING (
