-- Migration: User Assessment History
-- Description: Track assessment results over time for authenticated users
-- Created: 2025-01-18
-- Best Practice: Event sourcing pattern for tracking user progress
...
CREATE TABLE IF NOT EXISTS user_assessment_history (
  ...
  assessment_version VARCHAR(20) DEFAULT 'v1.0', -- Track assessment version for analytics

  -- Results
  score INTEGER NOT NULL,
  max_score INTEGER NOT NULL,
  percentage DECIMAL(5,2) GENERATED ALWAYS AS (ROUND((score::DECIMAL / NULLIF(max_score, 0)) * 100, 2)) STORED,
  level VARCHAR(50), -- e.g., 'low', 'moderate', 'high', 'severe'
  severity_score INTEGER CHECK (severity_score >= 0 AND severity_score <= 100),

  -- Detailed results
  assessment_results JSONB NOT NULL DEFAULT '{}',
  recommendations JSONB DEFAULT '[]',
  question_responses JSONB, -- Individual question answers for detailed analysis

  -- Context
  completed_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  duration_seconds INTEGER, -- Time taken to complete assessment
  device_type VARCHAR(50), -- 'mobile', 'tablet', 'desktop'

  -- Comparison with previous assessments
  previous_assessment_id UUID REFERENCES user_assessment_history(id),
  score_change INTEGER, -- Calculated difference from previous
  improvement_percentage DECIMAL(5,2),

  -- Flags
  is_baseline BOOLEAN DEFAULT FALSE, -- First assessment of this type
  flagged_for_review BOOLEAN DEFAULT FALSE, -- High-risk results
  ...
  -- HIPAA compliance
  phi_accessed_by JSONB DEFAULT '[]', -- Track who accessed this PHI
  retention_required_until TIMESTAMPTZ DEFAULT (NOW() + INTERVAL '7 years') -- HIPAA requirement
);
...
-- Policy: Providers can view patient assessments in their organization
CREATE POLICY "Providers can view organization patient assessments"
  ON user_assessment_history
  FOR SELECT
  TO authenticated
  USING (
    EXISTS (
      ...
      WHERE uom1.user_id = auth.uid()
        AND uom2.user_id = user_assessment_history.user_id
...
-- Create materialized view for assessment trends (performance optimization)
...
-- Function to refresh trends (can be called via cron job)
...
COMMENT ON COLUMN user_assessment_history.phi_accessed_by IS 'Audit trail of who accessed this protected health information';
